    else if(in->topicType == "anm_msgs/CommandCheckingReport")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::CommandCheckingReport>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__CommandCheckingReport, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/DynamicObstacle")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::DynamicObstacle>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__DynamicObstacle, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/DynamicObstacleList")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::DynamicObstacleList>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__DynamicObstacleList, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/HMIReport")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::HMIReport>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__HMIReport, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/NearestAnmWaypoint")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::NearestAnmWaypoint>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__NearestAnmWaypoint, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/PathState")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::PathState>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__PathState, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/ShuttleRequest")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::ShuttleRequest>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__ShuttleRequest, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/SpiralPath")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::SpiralPath>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__SpiralPath, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/SystemState")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::SystemState>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__SystemState, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/V2XStopSign")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::V2XStopSign>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__V2XStopSign, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/V2XStopSignList")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::V2XStopSignList>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__V2XStopSignList, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/V2XTrafficLight")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::V2XTrafficLight>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__V2XTrafficLight, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/V2XTrafficLightList")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::V2XTrafficLightList>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__V2XTrafficLightList, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/V2XVehicle")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::V2XVehicle>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__V2XVehicle, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/V2XVehicleList")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::V2XVehicleList>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__V2XVehicleList, _1, subscriberProxy));
    }
    else if(in->topicType == "anm_msgs/VehicleState")
    {
        subscriberProxy->subscriber = nh->subscribe<anm_msgs::VehicleState>(in->topicName, in->queueSize, boost::bind(ros_callback__anm_msgs__VehicleState, _1, subscriberProxy));
    }
    ...
    else if(in->topicType == "dbw_mkz_msgs/AmbientLight")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::AmbientLight>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__AmbientLight, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/BrakeCmd")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::BrakeCmd>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__BrakeCmd, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/BrakeInfoReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::BrakeInfoReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__BrakeInfoReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/BrakeReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::BrakeReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__BrakeReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/FuelLevelReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::FuelLevelReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__FuelLevelReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/Gear")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::Gear>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__Gear, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/GearCmd")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::GearCmd>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__GearCmd, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/GearReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::GearReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__GearReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/HillStartAssist")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::HillStartAssist>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__HillStartAssist, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/Misc1Report")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::Misc1Report>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__Misc1Report, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/ParkingBrake")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::ParkingBrake>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__ParkingBrake, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/SteeringCmd")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::SteeringCmd>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__SteeringCmd, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/SteeringReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::SteeringReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__SteeringReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/SurroundReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::SurroundReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__SurroundReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/SuspensionReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::SuspensionReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__SuspensionReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/ThrottleCmd")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::ThrottleCmd>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__ThrottleCmd, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/ThrottleInfoReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::ThrottleInfoReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__ThrottleInfoReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/ThrottleReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::ThrottleReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__ThrottleReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/TirePressureReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::TirePressureReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__TirePressureReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/TurnSignal")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::TurnSignal>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__TurnSignal, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/TurnSignalCmd")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::TurnSignalCmd>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__TurnSignalCmd, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/TwistCmd")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::TwistCmd>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__TwistCmd, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/WatchdogCounter")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::WatchdogCounter>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__WatchdogCounter, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/WheelSpeedReport")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::WheelSpeedReport>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__WheelSpeedReport, _1, subscriberProxy));
    }
    else if(in->topicType == "dbw_mkz_msgs/Wiper")
    {
        subscriberProxy->subscriber = nh->subscribe<dbw_mkz_msgs::Wiper>(in->topicName, in->queueSize, boost::bind(ros_callback__dbw_mkz_msgs__Wiper, _1, subscriberProxy));
    }
